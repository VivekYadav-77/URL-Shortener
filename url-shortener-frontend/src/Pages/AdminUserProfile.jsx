...
const AdminUserProfilePage = () => {
  ...
  const { data: userData, isLoading: loadingUser } = useGetUserProfileQuery(
    id,
    { skip: !id },
  );
  const { data: urls = [], isLoading: loadingUrls } = useGetUserUrlsQuery(id, {
    skip: !id,
  });
  ...
  const handleConfirmAction = async () => {
    try {
      if (isBlocked) {
        ...
        toast("User blocked successfully", "error");
      }
      setShowModal(false);
    } catch (err) {
      toast("Administrative action failed", "error");
    }
  };

  const handleEnable = async (urlId) => {
    try {
      await enableUrl(urlId).unwrap();
      ...
      >
        ...
        {deleteConfirmId && (
          <div className="fixed inset-0 z-110 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div
              className={`w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl border animate-in zoom-in duration-300 ${isDark ? "bg-[#0A0A0A] border-white/10" : "bg-white border-gray-200"} text-center`}
            >
              ...
              <p
                className={`mb-8 text-sm leading-relaxed ${isDark ? "text-gray-400" : "text-gray-500"}`}
              >
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className={`flex-1 py-4 rounded-2xl font-bold border transition-colors ${isDark ? "border-white/10 hover:bg-white/5 text-gray-400" : "border-gray-200 hover:bg-gray-50 text-gray-600"}`}
                >
                  Cancel
                </button>
                <button
                  onClick={handleDelete}
                  className="flex-1 bg-red-600 hover:bg-red-700 text-white py-4 rounded-2xl font-black shadow-xl shadow-red-900/20 transition-all active:scale-95"
                >
                  Delete Forever
                </button>
              </div>
        ...
        {showModal && (
          <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-300">
            <div
              className={`w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl border animate-in zoom-in duration-300 ${
                isDark
            ...
            >
              ...
              <p
                className={`mb-6 text-sm leading-relaxed ${isDark ? "text-gray-400" : "text-gray-500"}`}
              >
                {isBlocked
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold transition-all ${
              isDark
                ...
                : "bg-white border shadow-sm hover:bg-gray-50 text-gray-700"
            }`}
          >
        ...
        {loadingUser ? (
          ...
          <div
            className={`relative overflow-hidden p-8 rounded-[2.5rem] mb-10 border shadow-2xl transition-all ${
          ...
          >
            <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8">
              <div className="flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
                <div
                  className={`h-24 w-24 rounded-3xl flex items-center justify-center text-4xl font-black shadow-2xl rotate-3 transform transition hover:rotate-0 ${
                    isBlocked
                      ? "bg-red-600"
                      : isDark
                ...
                <div className="flex-1">
                  ...
                  <p
                    className={`text-lg mb-4 font-medium ${isDark ? "text-gray-400" : "text-gray-500"}`}
                  >
                    {user?.email}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-4 justify-center md:justify-start items-center">
                    <div
                      className={`px-4 py-2 rounded-2xl border ${isDark ? "bg-black/40 border-white/10" : "bg-gray-100 border-gray-200 text-gray-700"}`}
                    >
              ...
              <button
                ...
                className={`w-full lg:w-auto flex items-center justify-center gap-3 px-10 py-5 rounded-4xl font-black transition-all active:scale-95 shadow-2xl ${
                  isBlocked
            ...
            <div
              className={`absolute -right-20 -top-20 h-64 w-64 rounded-full blur-[120px] ${isBlocked ? "bg-red-600/20" : "bg-blue-600/10"}`}
            ></div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          ...
          <StatCard
            title="Archived"
            value={
              urls.filter((u) => ["deleted", "expired"].includes(u.status))
                .length
            }
            icon={<Clock />}
            color="red"
        ...
        >
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-250">
              <thead
                className={`${isDark ? "bg-white/5" : "bg-gray-50"} text-[10px] uppercase font-black text-gray-500 tracking-widest`}
              >
                <tr>
              ...
              >
                {loadingUrls ? (
                  <tr>
                    <td
                      colSpan="6"
                      className="p-20 text-center animate-pulse font-bold text-blue-500"
                    >
                      Retrieving user link data...
                    </td>
                  </tr>
                ) : urls.length === 0 ? (
                  <tr>
                    <td
                      colSpan="6"
                      className="p-20 text-center text-gray-500 font-medium italic"
                    >
                      No link assets found for this profile.
                    </td>
                  </tr>
                ) : (
                  urls.map((url) => (
                    ...
                    >
                      <td className="p-6 max-w-xs">
                        ...
                        <div
                          className={`text-[10px] font-mono ${isDark ? "text-gray-500" : "text-gray-400"}`}
                        >
                        ...
                        <div className="space-y-1">
                          {url.disabledAt && (
                            <div className="text-yellow-500 text-[10px] font-bold flex items-center gap-1">
                              <AlertCircle size={10} /> Disabled by{" "}
                              {url.disabledByRole}
                            </div>
                          )}
                          {url.deletedAt && (
...
const StatusBadge = ({ status, isDark }) => {
  ...
  return (
    <span
      className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border shadow-sm ${styles[status]}`}
    >
      {status}
    </span>
  );
...
const StatCard = ({ title, value, icon, color, isDark }) => {
  ...
  return (
    <div
      className={`p-8 rounded-[2.5rem] border transition-all hover:scale-[1.02] active:scale-95 cursor-default ${
        isDark
