      ...
      <div className="relative w-full max-w-md flex flex-col items-center">
        ...
        <div className="relative h-40 w-full flex items-center justify-center mb-4">
          ...
          <motion.div
            ...
            className={`relative z-20 w-24 h-24 rounded-4xl flex items-center justify-center border-4 shadow-2xl transition-all ${isDark ? "bg-blue-600 border-white/10 shadow-blue-900/40" : "bg-blue-600 border-white shadow-blue-200"}`}
          >
            <LogoIcon className="text-white w-12 h-12" />
            <motion.div
              animate={{ scale: [1, 1.3, 1], opacity: [0.3, 0, 0.3] }}
              transition={{ duration: 3, repeat: Infinity }}
              className="absolute inset-0 rounded-4xl border-2 border-blue-300/50 pointer-events-none"
            />
          </motion.div>
        </div>
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-500 text-[10px] font-black uppercase tracking-widest mb-4">
            <Sparkles size={12} />{" "}
            {isSuccess ? "Synthesis Initiated" : "Account Synthesis"}
          </div>
          <h1
            className={`text-5xl font-black tracking-tighter ${isDark ? "text-white" : "text-slate-900"}`}
          >
            {isSuccess ? "Verify " : "Join "}
            <span className="text-blue-600">
              {isSuccess ? "Identity." : "Shortly."}
            </span>
          </h1>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          {isSuccess ? (
            <motion.div
              ...
              className={`w-full rounded-[3rem] p-10 border shadow-2xl space-y-7 backdrop-blur-xl text-center ${isDark ? "bg-white/2 border-white/10 shadow-black" : "bg-white border-slate-200 shadow-slate-200/50"}`}
            >
              <div className="flex justify-center">
                <div className="bg-blue-500/10 p-5 rounded-full text-blue-500 animate-pulse">
                  <Mail size={40} />
                </div>
              </div>
              <p
                className={`text-sm font-bold leading-relaxed ${isDark ? "text-slate-400" : "text-slate-600"}`}
              >
                {data?.message ||
              ...
              <button
                ...
                className="w-full py-5 rounded-2xl font-black text-xs uppercase tracking-widest bg-blue-600 text-white shadow-xl shadow-blue-600/20"
              >
                Go to Login Portal
              </button>
            </motion.div>
            ...
            isSynthesized && (
              <motion.form
                ...
                className={`w-full rounded-[3rem] p-8 sm:p-10 border shadow-2xl space-y-6 backdrop-blur-xl ${isDark ? "bg-white/2 border-white/10 shadow-black" : "bg-white border-slate-200 shadow-slate-200/50"}`}
              >
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className={`w-full px-6 py-4 rounded-2xl border font-bold text-sm outline-none transition-all ${isDark ? "bg-black/40 border-white/10 text-white focus:border-blue-500" : "bg-slate-50 border-slate-200 focus:border-blue-500"}`}
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-blue-500 flex items-center gap-2 px-1">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`w-full px-6 py-4 pr-14 rounded-2xl border font-bold text-sm outline-none transition-all ${isDark ? "bg-black/40 border-white/10 text-white focus:border-blue-500" : "bg-slate-50 border-slate-200 focus:border-blue-500"}`}
                      />
                      <button
                        type="button"
