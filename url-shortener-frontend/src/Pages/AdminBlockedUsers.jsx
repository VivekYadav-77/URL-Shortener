...
const AdminBlockedUsersPage = () => {
  ...
  const handleUnblock = async (id, name) => {
    if (window.confirm(`Are you sure you want to reinstate access for ${name}?`)) {
      try {
      ...
      <div className={`min-h-screen p-4 md:p-8 transition-colors duration-500 ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className={`p-6 rounded-3xl border ${isDark ? "bg-white/5 border-white/10" : "bg-white border-gray-200 shadow-sm"}`}>
            <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-1">Total Restricted</span>
            <span className="text-3xl font-black text-red-500">{blockedUsers.length}</span>
          </div>
          <div className={`p-6 rounded-3xl border ${isDark ? "bg-white/5 border-white/10" : "bg-white border-gray-200 shadow-sm"}`}>
            <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-1">Search Results</span>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map(i => <div key={i} className="h-64 animate-pulse bg-white/5 rounded-[2.5rem]"></div>)}
          </div>
        ) : filteredUsers.length === 0 ? (
          <div className={`p-20 text-center rounded-[3rem] border-2 border-dashed ${isDark ? "border-white/10" : "border-gray-200"}`}>
            <UserX size={48} className="mx-auto mb-4 text-gray-500 opacity-20" />
            <p className="text-gray-500 font-bold italic">No blocked users matching your criteria.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredUsers.map((user) => (
              <div 
                ...
                className={`group relative overflow-hidden p-8 rounded-[2.5rem] border transition-all hover:scale-[1.02] ${
                  isDark ? "bg-white/5 border-white/10 hover:border-red-500/30" : "bg-white border-gray-200 shadow-xl shadow-gray-200/50"
                }`}
              >
                {/* TOP ROW: AVATAR & INFO */}
                ...
                {/* USER DETAILS */}
                <h3 className="text-xl font-black mb-1 truncate">{user.name}</h3>
                <div className="flex items-center gap-2 text-sm text-gray-500 mb-6 truncate font-medium">
                  <Mail size={14} /> {user.email}
                </div>

                {/* BLOCK DETAILS */}
                <div className={`p-4 rounded-2xl mb-8 space-y-3 ${isDark ? "bg-black/40" : "bg-gray-50"}`}>
                  <div className="flex items-start gap-3">
                    <Calendar size={14} className="mt-1 text-red-500" />
                    <div>
                      <span className="text-[10px] font-black uppercase text-gray-500 block">Restricted On</span>
                      <span className="text-xs font-bold">{new Date(user.blockedAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <MessageSquare size={14} className="mt-1 text-red-500" />
                    <div>
                      <span className="text-[10px] font-black uppercase text-gray-500 block">Enforcement Reason</span>
                      <span className="text-xs font-bold line-clamp-2">{user.reason || "No reason specified"}</span>
                    </div>
                  </div>
                </div>
