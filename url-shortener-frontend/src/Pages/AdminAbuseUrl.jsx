      ...
      <div className={`min-h-screen ${pageBg} transition-colors duration-300`}>
        {feedback && (
          <div className="fixed bottom-6 right-6 z-50 animate-bounce">
            <div
              className={`px-4 py-3 rounded-xl shadow-2xl text-sm font-bold border ${feedback.type === "success" ? "bg-green-600 text-white border-green-500" : "bg-red-600 text-white border-red-500"}`}
            >
              {feedback.msg}
            </div>
          </div>
        ...
        {selectedUrl && (
          <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
            ...
            >
              ...
              {(selectedUrl.status === "deleted" ||
                selectedUrl.status === "inactive") && (
                <div
                  className={`mb-6 p-4 rounded-xl border ${selectedUrl.status === "deleted" ? "bg-red-500/10 border-red-500/20" : "bg-yellow-500/10 border-yellow-500/20"}`}
                >
                  ...
                  <p className="text-xs mt-1 opacity-80">
                    By:{" "}
                    {selectedUrl.deletedByRole ||
                      selectedUrl.disabledByRole ||
                      "System"}
                  </p>
                </div>
              )}
              ...
              <div className="flex gap-4">
                <button
                  onClick={() => handleCopy(selectedUrl.originalUrl)}
        ...
        >
          {isLoading ? (
            <div className="p-16 text-center animate-pulse font-bold text-blue-500">
              Scanning Database...
            </div>
          ) : searchedUrls.length === 0 ? (
            <div className="p-16 text-center font-bold text-gray-500">
              No records found.
            </div>
          ) : (
                ...
                <tbody className={`divide-y ${border}`}>
                  {searchedUrls.map((url) => {
                    ...
                    const isAdminDeleted =
                      url.status === "deleted" && url.deletedByRole === "admin";
                    ...
                    return (
                      <tr
                        key={url._id}
                        className={`${rowHover} transition-colors ${isAdminDeleted ? "opacity-60 grayscale-[0.5]" : ""}`}
                      >
                        <td className="p-5">
                          <a
                            href={shortUrl}
                        ...
                        <td className="p-5 max-w-xs">
                          <p
                            ...
                            className={`text-sm truncate cursor-pointer hover:text-blue-500 font-medium ${softText}`}
                          >
                            {url.originalUrl}
                          </p>
                        </td>
                        <td className="p-5 text-center">
                          <span
                            className={`px-4 py-1 rounded-lg text-xs font-black border ${getSeverityBadge(url.abuseScore)}`}
                          >
                            {url.abuseScore}
                          </span>
                        </td>
