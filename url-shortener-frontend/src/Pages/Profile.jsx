...
const Profile = () => {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    if (name.trim() === user.name)
      return showFeedback("warning", "Name is already up to date");
    try {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    if (!currentPassword || !newPassword)
      return showFeedback("warning", "Please provide both passwords");
    if (currentPassword === newPassword)
      return showFeedback("warning", "New password must be different");
    if (newPassword.length < 8)
      return showFeedback("warning", "Security requirement: 8+ characters");
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 mb-16">
            <div className="relative group">
              <div
                className={`w-32 h-32 rounded-[2.5rem] flex items-center justify-center border-4 shadow-2xl transition-transform duration-500 group-hover:rotate-6 ${
                  isDark
                    ? "bg-linear-to-br from-blue-600 to-purple-700 border-white/10"
          ...
          <div ref={feedbackRef} className="h-20 mb-4">
            {feedback.message && (
              <div
                className={`flex items-center gap-3 p-5 rounded-3xl border shadow-2xl animate-in slide-in-from-top-4 duration-300 ${
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <form
              ...
              className={`p-8 md:p-10 rounded-[2.5rem] border shadow-2xl ${isDark ? "bg-[#0A0A0A] border-white/10" : "bg-white border-gray-200"}`}
            >
              <div className="flex items-center gap-3 mb-8">
                <div
                  className={`p-3 rounded-2xl ${isDark ? "bg-blue-500/10 text-blue-500" : "bg-blue-50 text-blue-600"}`}
                >
                  <UserCircle size={24} />
                </div>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div
                    className={`px-5 py-4 rounded-2xl border font-bold text-sm opacity-60 ${isDark ? "bg-black/40 border-white/5 text-gray-400" : "bg-gray-100 border-gray-200"}`}
                  >
                    {user.email}
                  </div>
                </div>
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 shadow-lg shadow-blue-600/20"
                >
                  {updatingProfile ? (
                    "Saving..."
                  ) : (
            ...
            >
              <div className="flex items-center gap-3 mb-8">
                <div
                  className={`p-3 rounded-2xl ${isDark ? "bg-red-500/10 text-red-500" : "bg-red-50 text-red-600"}`}
                >
                  <ShieldCheck size={24} />
                </div>
              ...
              <div className="space-y-6">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full bg-red-600 hover:bg-red-700 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 shadow-lg shadow-red-600/20"
                >
                  {changingPassword ? (
                    "Updating..."
                  ) : (
                    <>
                      <KeyRound size={16} /> Update Security
                    </>
                  )}
                </button>
              </div>
