...
import AdminLayout from "./AdminLayout"; // Switched to AdminLayout
...
const AdminProfile = () => {
  ...
  const handleProfileSubmit = async (e) => {
    e.preventDefault();
    if (name.trim() === user.name) return showFeedback("warning", "Identity parameters already synced.");
    try {
  ...
  const handlePasswordSubmit = async (e) => {
    e.preventDefault();
    if (newPassword.length < 8) return showFeedback("warning", "Security breach: Minimum 8 characters required.");
    ...
    try {
      ...
      showFeedback("error", err?.data?.message || "Encryption update rejected.");
    }
  };

  return (
    <AdminLayout>
      <div className={`min-h-screen px-6 py-12 transition-colors duration-700 ${isDark ? "bg-[#020202] text-white" : "bg-slate-50 text-slate-900"}`}>
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-16 border-b border-white/5 pb-10">
            <div className="flex items-center gap-8">
              <motion.div 
                ...
                className={`w-32 h-32 rounded-3xl flex items-center justify-center border-4 shadow-[0_0_50px_rgba(37,99,235,0.2)] relative overflow-hidden ${
                  isDark ? "bg-black border-blue-500/30" : "bg-white border-blue-600"
                }`}
              >
                ...
                <motion.div 
                   ...
                   className="absolute top-0 left-0 right-0 h-[1px] bg-blue-400 opacity-50 shadow-[0_0_10px_#3b82f6]"
                />
              </motion.div>
            ...
            <div className="hidden lg:flex gap-4">
               {[ { l: "CPU", v: "14%" }, { l: "MEM", v: "2.4GB" }, { l: "LOGS", v: "SYNCED" }].map((s, i) => (
                 <div key={i} className={`px-6 py-3 rounded-2xl border ${isDark ? "bg-white/5 border-white/10" : "bg-white border-slate-200"}`}>
              ...
              {feedback.message && (
                <motion.div 
                  ...
                  className={`flex items-center gap-3 p-4 rounded-2xl border backdrop-blur-md shadow-2xl ${
                    feedback.type === "success" ? "bg-blue-500/10 border-blue-500/20 text-blue-500" : "bg-red-500/10 border-red-500/20 text-red-500"
                  }`}
                >
                  <Fingerprint size={20} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            >
              <div className="flex items-center gap-4 mb-10">
                ...
                <h2 className="text-2xl font-black tracking-tight uppercase">Admin Identity</h2>
              </div>

              <div className="space-y-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500 px-1">Network Identity (Read-Only)</label>
                  <div className={`px-6 py-4 rounded-2xl border font-mono text-sm ${isDark ? "bg-black/60 border-white/5 text-slate-500" : "bg-slate-100 border-slate-200"}`}>
                    {user.email}
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500 px-1">Full Designation</label>
                ...
                <button type="submit" disabled={updatingProfile} className="w-full bg-blue-600 hover:bg-blue-500 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-3 active:scale-95 shadow-xl shadow-blue-600/20">
                  {updatingProfile ? "SYCHRONIZING..." : <><Save size={18} /> Deploy Identity</>}
                </button>
              </div>
            </motion.form>
            ...
            >
              <div className="flex items-center gap-4 mb-10">
                ...
                <h2 className="text-2xl font-black tracking-tight uppercase">Security Protocol</h2>
              </div>

              <div className="space-y-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-red-500 px-1">Current Secret Key</label>
                  <div className="relative">
                    ...
                    />
                    <button type="button" onClick={() => setShowCurrent(!showCurrent)} className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-500 hover:text-red-500 transition-colors">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-red-500 px-1">New Master Key</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-14 pr-14 py-4 rounded-2xl border font-bold text-sm outline-none transition-all ${isDark ? "bg-black/40 border-white/10 text-white focus:border-red-600" : "bg-slate-50 border-slate-200 focus:border-red-600"}`}
                    />
                    <button type="button" onClick={() => setShowNew(!showNew)} className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-500 hover:text-red-500 transition-colors">
                      {showNew ? <EyeOff size={18} /> : <Eye size={18} />}
                    </button>
                  </div>
                </div>

                <button type="submit" disabled={changingPassword} className="w-full bg-slate-900 dark:bg-red-600 hover:bg-black dark:hover:bg-red-500 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-3 active:scale-95 shadow-xl">
                  {changingPassword ? "ENCRYPTING..." : <><ShieldAlert size={18} /> Override Security</>}
                </button>
              </div>
            </motion.form>
